...
export class DateTimeUtils {
	...
	// Get next hour
	static getNextHour(): DateTime {
		const now = this.getNow();
		const nextHour = now.plus({ hour: 1 });
		return nextHour.set({ minute: 0, second: 0, millisecond: 0 });
	}

	// Get next hour string
	static getNextHourString(): string {
		const datetime = this.getNextHour();
		return String(datetime.toFormat("HH.mm"));
	}
	...
	static getDatesBetween(from: string | DateTime, to: string | DateTime): string[] {
		const fromDate = typeof from === "string" ? this.convertStringToDateTime(from) : from;
		const toDate = typeof to === "string" ? this.convertStringToDateTime(to) : to;
		const result: string[] = [];

		if (fromDate && toDate && fromDate.isValid && toDate.isValid) {
			const duration = Math.abs(fromDate.diff(toDate, ["days"]).days);
			for (let i = 0; i < duration; i++) {
				const date = fromDate.plus({ days: i });
				result.push(this.convertDateTimeToString(date));
			}
			return result;
		}
	...
	static formatDateRange(start: DateTime, end: DateTime): string {
		if (start.equals(end)) {
			return start.toLocaleString({ day: "numeric", month: "long" });
		} else if (start.month === end.month) {
			return `${start.toLocaleString({ day: "numeric" })}-${end.toLocaleString({ day: "numeric", month: "long" })}`;
		} else {
